...
import (
	"errors"
	"github.com/sax1412/go-logger/utils"
	"os"
	"path"
	"reflect"
	"strings"
...
func (fw *FileWriter) writeByConfig(config *FileConfig, loggerMsg *loggerMessage) error {
	...
	if fw.fileChange == 0 {
		filenameSuffix := path.Ext(fw.filename)
		dirIndex := strings.LastIndex(fw.filename, "/")
		extIndex := strings.LastIndex(fw.filename, ".")
		logDir := string(fw.filename[:dirIndex+1]) + time.Now().Format("2006/01/02/")
		hour := time.Now().Format("2006010215")
		newPath := logDir + string(fw.filename[dirIndex+1:extIndex]) + "_" + string(hour[8:]) + filenameSuffix
		fw.filename = newPath
		error := os.MkdirAll(logDir, 0777)
		if error != nil {
			return error
		}
		fw.initFile()
		fw.fileChange = 1
		fw.projectDir = string(fw.filename[:dirIndex+1])
	}
...
func (fw *FileWriter) sliceByDate(dataSlice string) error {
	...
	if (dataSlice == FILE_SLICE_DATE_HOUR) &&
		...
		isHaveSlice = true
		//源码改动
		extIndex := strings.LastIndex(filename, "_")
		dirIndex := strings.LastIndex(filename, "/")
		logDir := fw.projectDir + time.Now().Format("2006/01/02/")
		hour := time.Now().Format("2006010215")
		...
		oldFilename = logDir + string(filename[dirIndex+1:extIndex]) + "_" + string(hour[8:]) + filenameSuffix
	}

	if isHaveSlice == true {
		//close file handle
		fw.writer.Close()
		fw.filename = oldFilename
		err := fw.initFile()
		if err != nil {
			return err
		}
